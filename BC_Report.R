...
bc_odisha = read_xlsx(here("data/4109450_STATE_BOUQUET_RPT_WB_NOV20.XLS"),skip = 3,)
...
bc_odisha_filterred = bc_odisha %>%
  select(Broadcaster.Name,Bouquet,Monthly.Subs.of.the.Channel) %>% distinct()%>% na.omit()
bc_name = bc_odisha %>%
...
write.csv(active_pivot, "WB_bc.csv", row.names = FALSE)