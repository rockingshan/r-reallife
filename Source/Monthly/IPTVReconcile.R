...
drmReportMod = drmReport %>% select(raw_cas_sn,signature,packages,status)
...
drmReportMod = drmReportMod %>% unite(combined, c('raw_cas_sn','packages','status'),sep = "|",remove = F)

finalData = merge(smsReportMod,drmReportMod,by.x = 'userid',by.y = 'raw_cas_sn',all.x = T)
