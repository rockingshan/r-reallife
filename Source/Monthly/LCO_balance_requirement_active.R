...
lco_price = price %>% filter(Lco.Price.Status == "A") %>% filter(Entity.Code == "MD0501") %>%  unite(combined, c("Entity.Code","Plan.Code")) %>% select(combined,Price) %>% unique()
...
total_act_date_price = merge(total_act_date_asc,active_price,all.x = T)
