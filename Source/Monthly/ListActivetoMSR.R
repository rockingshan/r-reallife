...
active_to_msr_format <- function() {
  ...
  list_active1 = list_active %>%
    ...
    filter(!(PLAN_NAME == 'zDelete Pairing')) %>%
    filter(!(PLAN_NAME == 'All Channel Special Pack')) %>%
    unique()
  pack_details = read.csv(file.choose()) %>%
    select(Service.Code, Broadcaster) %>%
    unique()
  ...
  list_active1$SERVICE_NAME <- ifelse(
    grepl("^MBIL", list_active1$SERVICE_CODE),
    list_active1$PLAN_NAME,
    list_active1$SERVICE_NAME
  )
  colnames(list_active1)[8] = "Plan.Name"
  listActiveBroadcast = merge(
    list_active1,
    pack_details,
    by = "SERVICE_CODE",
    all.x = T
  ) %>%
    filter(!(is.na(Broadcaster.Name)))
  listActiveCount = listActiveBroadcast %>%
    group_by(LCO_CITY, Broadcaster.Name, Plan.Name, SERVICE_NAME) %>%
    summarise(Active.Cust = n())
...
msrAllCombo = msrAllCombo %>%
  mutate(
    Monthly.Subs.of.the.Channel = rowMeans(select(
      msrAllCombo,
      starts_with("No.of")
    ))
  )
...
msrAlacarteSub = msrAlacarte %>%
  filter((SERVICE_NAME %in% c("Sidharth Odia Pack-1", "Odisha Tv Bouqet 1")))
msrAlacarte = msrAlacarte %>%
  filter(!(SERVICE_NAME %in% c("Sidharth Odia Pack-1", "Odisha Tv Bouqet 1")))
...
####work on Bouqets#######
msrBouquet = msrBouquet %>%
  filter(
    !(Broadcaster.Name %in%
      c("ABP News Network Pvt Limited", "Free to AIR", "Republic TV", "DD"))
  )
...
msrBouquet_new_7 = msrBouquet %>%
  filter(Bouquet %in% bouquet_names$Bouquet) %>%
  select(LCO_CITY, Plan.Name, No.of.Subs.On.7th.Day) %>%
  unique()
msrBouquet_new_14 = msrBouquet %>%
  filter(Bouquet %in% bouquet_names$Bouquet) %>%
  select(LCO_CITY, Plan.Name, No.of.Subs.On.14th.Day) %>%
  unique()
msrBouquet_new_21 = msrBouquet %>%
  filter(Bouquet %in% bouquet_names$Bouquet) %>%
  select(LCO_CITY, Plan.Name, No.of.Subs.On.21st.Day) %>%
  unique()
msrBouquet_new_28 = msrBouquet %>%
  filter(Bouquet %in% bouquet_names$Bouquet) %>%
  select(LCO_CITY, Plan.Name, No.of.Subs.On.28th.Day) %>%
  unique()
#msrBouquet_new_avg = msrBouquet %>% select(LCO_CITY,Plan.Name,Monthly.Subs.of.the.Channel) %>% unique()

msrBouquet_new_7_pk = merge(msrBouquet_new_7, singlepack_7, all.y = F) %>%
  unique() %>%
  unite(combined, c('Plan.Name', 'Bouquet'), sep = "|")
msrBouquet_new_14_pk = merge(msrBouquet_new_14, singlepack_14, all.y = F) %>%
  unique() %>%
  unite(combined, c('Plan.Name', 'Bouquet'), sep = "|")
msrBouquet_new_21_pk = merge(msrBouquet_new_21, singlepack_21, all.y = F) %>%
  unique() %>%
  unite(combined, c('Plan.Name', 'Bouquet'), sep = "|")
msrBouquet_new_28_pk = merge(msrBouquet_new_28, singlepack_28, all.y = F) %>%
  unique() %>%
  unite(combined, c('Plan.Name', 'Bouquet'), sep = "|")

msrBouquet_combo_new = merge(msrBouquet_new_7_pk, msrBouquet_new_14_pk, all = T)
msrBouquet_combo_new = merge(
  msrBouquet_combo_new,
  msrBouquet_new_21_pk,
  all = T
)
msrBouquet_combo_new = merge(
  msrBouquet_combo_new,
  msrBouquet_new_28_pk,
  all = T
) %>%
  separate(combined, into = c("Plan.Name", "Bouquet"), sep = "\\|")

msrBouquet_combo_new[is.na(msrBouquet_combo_new)] <- 0
msrBouquet_combo_new$No.of.Subs.On.7th.Day = as.numeric(
  msrBouquet_combo_new$No.of.Subs.On.7th.Day
)
msrBouquet_combo_new$No.of.Subs.On.14th.Day = as.numeric(
  msrBouquet_combo_new$No.of.Subs.On.14th.Day
)
msrBouquet_combo_new$No.of.Subs.On.21st.Day = as.numeric(
  msrBouquet_combo_new$No.of.Subs.On.21st.Day
)
msrBouquet_combo_new$No.of.Subs.On.28th.Day = as.numeric(
  msrBouquet_combo_new$No.of.Subs.On.28th.Day
)
msrBouquet_combo_new = msrBouquet_combo_new %>%
  mutate(
    Monthly.Subs.of.the.Channel = rowMeans(select(
      msrBouquet_combo_new,
      starts_with("No.of")
...
msrBouquet_combo_new_bouq = msrBouquet_combo_new %>%
  filter(X == 'Bouquet') %>%
  select(
    LCO_CITY,
    Broadcaster.Name,
...
msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "WBD Kids SD NT2"
] <- "WBD Life SD NT2"
msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "WBD Kids HD NT2"
] <- "WBD Life HD NT2"
msrBouquet_bq_all$Plan.Name[
  msrBouquet_bq_all$Plan.Name == "WBD Kids SD NT2 @ 14"
] <- "WBD Life SD NT2 @ 14"
msrBouquet_bq_all$Plan.Name[
  msrBouquet_bq_all$Plan.Name == "WBD Kids HD NT2 @ 22"
] <- "WBD Life HD NT2 @ 22"
msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "COLORS WALA BANGLA BUDGET HD NT2"
] <- "COLORS WALA BANGLA VALUE HD NT2"
msrBouquet_bq_all$Plan.Name[
  msrBouquet_bq_all$Plan.Name == "COLORS WALA BANGLA BUDGET HD NT2 @ 32"
] <- "COLORS WALA BANGLA VALUE HD NT2 @ 54"
msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "Colors WalaHindi Budget Plus NT2"
] <- "Colors WalaHindi Value Plus NT2"
msrBouquet_bq_all$Plan.Name[
  msrBouquet_bq_all$Plan.Name == "Colors WalaHindi Budget Plus NT2 @ 25"
] <- "Colors WalaHindi Value Plus NT2 @ 34"

msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "Zee All-In-One Pack Bangla HD (A)"
] <- "Zee All-In-One Pack Bangla HD A"
msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "Zee All-in-One Pack Hindi SD (A)"
] <- "Zee All-in-One Pack Hindi SD A"
msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "Zee All-In-One Pack Hindi HD (A)"
] <- "Zee All-In-One Pack Hindi HD A"
msrBouquet_bq_all$Plan.Name[
  msrBouquet_bq_all$Plan.Name == "Happy India Smart -Bangla 2.0 @ 56"
] <- "Happy India Smart - Bangla 2.0 @ 56"
msrBouquet_bq_all$Bouquet[
  msrBouquet_bq_all$Bouquet == "Happy India Smart -Bangla 2.0"
] <- "Happy India Smart - Bangla 2.0"
...
msrBouqRpt = msrBouquet_bq_all %>%
  ...
  summarize(
    ...
    'Active_21st' = sum(No.of.Subs.On.21st.Day),
...
msrAlacarte_filterred = msrAlacarte %>%
  ...
  distinct() %>%
  na.omit()
msrAlacarte_singlecas = msrBouquet_combo_new %>%
  filter(X == 'Alacarte') %>%
  select(
    LCO_CITY,
    Broadcaster.Name,
...
msrAlacarte_final <- msrAlacarte_final %>%
  filter(
    !(Channel == "Sun Bangla HD" &
      Broadcaster.Name == "Eenadu Television Pvt Ltd")
  )
...
saveWorkbook(wb1, "Output/MSR_Report_all_Mar26.xlsx", overwrite = TRUE)
...
saveWorkbook(wb2, "Output/MSR_Report_Planwise_all_Mar26.xlsx", overwrite = TRUE)
...
saveWorkbook(wb3, "Output/MSR_Report_Areawise_all_Mar26.xlsx", overwrite = TRUE)
...
colnames(iptvreport) <- c(
  "Plan.Name",
  "Code",
  "No.of.Subs.On.7th.Day",
  "No.of.Subs.On.14th.Day.14TH_DAY",
  "No.of.Subs.On.21st.Day",
  "No.of.Subs.On.28th.Day.28TH_DAY"
)
iptvreport = iptvreport %>%
  group_by(Code) %>%
...
iptv_nw_14 = iptvreport %>%
  select(Code, No.of.Subs.On.14th.Day.14TH_DAY) %>%
  unique()
iptv_nw_21 = iptvreport %>% select(Code, No.of.Subs.On.21st.Day) %>% unique()
iptv_nw_28 = iptvreport %>%
  select(Code, No.of.Subs.On.28th.Day.28TH_DAY) %>%
  unique()

iptv_nw_7_pk = merge(iptv_nw_7, singlepack_7, all.y = T) %>%
  unique() %>%
  unite(combined, c('Code', 'Bouquet'), sep = "|")
iptv_nw_14_pk = merge(iptv_nw_14, singlepack_14, all.y = F) %>%
  unique() %>%
  unite(combined, c('Code', 'Bouquet'), sep = "|")
iptv_nw_21_pk = merge(iptv_nw_21, singlepack_21, all.y = F) %>%
  unique() %>%
  unite(combined, c('Code', 'Bouquet'), sep = "|")
iptv_nw_28_pk = merge(iptv_nw_28, singlepack_28, all.y = F) %>%
  unique() %>%
  unite(combined, c('Code', 'Bouquet'), sep = "|")
iptv_combo = merge(
  iptv_nw_7_pk,
...
iptv_combo = merge(iptv_combo, iptv_nw_28_pk, all = T) %>%
...
iptv_combo$No.of.Subs.On.14th.Day.14TH_DAY = as.numeric(
  iptv_combo$No.of.Subs.On.14th.Day.14TH_DAY
)
iptv_combo$No.of.Subs.On.21st.Day = as.numeric(
  iptv_combo$No.of.Subs.On.21st.Day
)
iptv_combo$No.of.Subs.On.28th.Day.28TH_DAY = as.numeric(
  iptv_combo$No.of.Subs.On.28th.Day.28TH_DAY
)
iptv_combo = iptv_combo %>%
  mutate(
    Monthly.Subs.of.the.Channel = rowMeans(select(
      iptv_combo,
      starts_with("No.of")
...
iptv_combo_bouq$No.of.Subs.On.14th.Day.14TH_DAY = as.numeric(
  iptv_combo_bouq$No.of.Subs.On.14th.Day.14TH_DAY
)
iptv_combo_bouq$No.of.Subs.On.21st.Day = as.numeric(
  iptv_combo_bouq$No.of.Subs.On.21st.Day
)
iptv_combo_bouq$No.of.Subs.On.28th.Day.28TH_DAY = as.numeric(
  iptv_combo_bouq$No.of.Subs.On.28th.Day.28TH_DAY
)
...
active_pivot = iptv_combo_bouq %>%
  ...
  summarize(
    'Active_7th' = sum(No.of.Subs.On.7th.Day),
    'Active_14th' = sum(No.of.Subs.On.14th.Day.14TH_DAY),
    'Active_21st' = sum(No.of.Subs.On.21st.Day),
    'Active_28th' = sum(No.of.Subs.On.28th.Day.28TH_DAY),
    'Average' = sum(Monthly.Subs.of.the.Channel)
  )
...
write.xlsx(
  ...
  file = "Output/IPTV_MSR__all_Mar26.xlsx",
  sheetName = "Bouquet",
  row.names = FALSE
)
write.xlsx(
...
# ##NTO report all
# write.xlsx(as.data.frame(od_bq_rpt_pl), file="Output/IPTV_MSR_Planwise_all_Mar26.xlsx", sheetName="Bouquet", row.names=FALSE)
# write.xlsx(as.data.frame(od_al_rpt_pl), file="Output/IPTV_MSR_Planwise_all_Mar26.xlsx", sheetName="Alacarte", append=TRUE, row.names=FALSE)
#
...
bq_report1 = bq_report %>%
  filter(!(CHANNEL_NAME_5 == '')) %>%
  filter(!(CHANNEL_NAME_5 == 'zDelete Pairing')) %>%
  select(Cust.Id, Week, Lco.Code) %>%
  unique()
bq_report_count = bq_report1 %>%
  group_by(Lco.Code, Week) %>%
  summarise(Active.Cust = n())
bq_report_proper = bq_report_count %>%
  pivot_wider(names_from = Week, values_from = Active.Cust) %>%
  replace_na(list(`1` = 0, `2` = 0, `3` = 0, `4` = 0))
colnames(bq_report_proper) <- c(
  "Lco.Code",
  "No.of.Subs.On.7th.Day",
  "No.of.Subs.On.14th.Day",
  "No.of.Subs.On.21st.Day",
...
bq_report_proper = bq_report_proper %>%
...
write.csv(bq_report_area, "Weekly_Active_subs_Mar26.csv", row.names = F)
