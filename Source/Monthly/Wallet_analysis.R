...
wallet_filt = filter(wallet, Credit.Document.Type == "INVOICE") %>%
  select(
    Customer.Nbr,
    Customer.Name,
    Unique.Id,
...
wallet_serv_bc = wallet_oth %>%
  mutate(Broadcaster.part_WO_TAX = ((Amount.Debit / 1.18) * 0.889))
wallet_serv_bc$Broadcaster.part_WO_TAX = round(
  wallet_serv_bc$Broadcaster.part_WO_TAX,
  digits = 2
)

wallet_servc_bc_dr = wallet_dir %>%
  mutate(Broadcaster.part_WO_TAX = ((Amount.Debit / 1.18) * 0.8))
wallet_servc_bc_dr$Broadcaster.part_WO_TAX = round(
  wallet_servc_bc_dr$Broadcaster.part_WO_TAX,
  digits = 2
)
wallet_final = rbind(wallet_serv_bc, wallet_servc_bc_dr)

wallet_final_calc = wallet_final %>%
  select(Service.Name, Amount.Debit, Broadcaster, Broadcaster.part_WO_TAX) %>%
  group_by(Service.Name, Broadcaster) %>%
  summarise(
    Total_ded_With_TAX = sum(Amount.Debit),
    Broadcaster_deduct_w_o_tax = sum(Broadcaster.part_WO_TAX)
  )
write.csv(wallet_final_calc, "SERVICEwise_amount_May22.csv")

wallet_dpo_all = wallet_dpo %>%
  group_by(Plan.Details) %>%
  summarise(TotalWallet = sum(Amount.Debit))
write.csv(wallet_dpo_all, "DPOPack_amount_May22.csv")

wallet_plan = wallet_filt %>%
  filter((Service.Name == '')) %>%
  select(Plan.Details, Amount.Debit) %>%
  group_by(Plan.Details) %>%
  summarise(Total_ded_W_TAX = sum(Amount.Debit))
...
my_df <- wallet1 %>%
  mutate(
    Transactions.DateTime = as.POSIXct(
      Transaction.Date,
