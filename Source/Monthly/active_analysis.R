...
ls_bq_com = list_bouquet_dated %>% select(Cust.Id,Set.Top.Box,Service.Cas.Code,CHANNEL_NAME_5) %>% unite(combined, c("Set.Top.Box","Service.Cas.Code"))
...
ls_al_com = list_alacarte %>% select(Cust.Id,Set.Top.Box,Service.Cas.Code,Channel.Name) %>% unite(combined, c("Set.Top.Box","Service.Cas.Code"))
...
write.csv(all_lco,"LCO_DPO_count_November25.csv")
...
write.csv(royal_pivot, "325_pack_status_November25.csv",row.names = F)
...
####ABV CAS Data Make ####
list_active <- mq_active_report()
abvData = list_active %>% filter(VC.length == "ABV") %>% select(STB,VC,CASCODE) %>% unique()
