...
list_active_flt = list_active %>%
  filter(SERVICE_CODE != '') %>%
  filter(!(PLAN_NAME == 'DPO Promotional Bundle'))

##BROADCASTERWISE DATA
list_active_bc = merge(
  list_active_flt,
  broadcaster,
  by.x = "SERVICE_CODE",
  by.y = 'Service.Code',
...
list_active_bc <- list_active_bc %>%
  ...
  unique()
list_active_piv = list_active_bc %>%
...
ls_bq_com = list_bouquet_dated %>%
  select(Cust.Id, Set.Top.Box, Service.Cas.Code, CHANNEL_NAME_5) %>%
  unite(combined, c("Set.Top.Box", "Service.Cas.Code"))

ls_act_com = list_active_flt %>%
  select(STB, CASCODE, ENTITY_CODE) %>%
  unite(combined, c("STB", "CASCODE"))

bouquet_diff = merge(ls_bq_com, ls_act_com, all.x = T, all.y = F)
bouquet_diff_flt = bouquet_diff %>%
  filter(is.na(ENTITY_CODE)) %>%
  separate(combined, c("STB", "cascode"))

write.csv(
  bouquet_diff_flt,
  "Bouquets_in_MSR_notin_ListActive.csv",
  row.names = F
)
...
ls_al_com = list_alacarte %>%
  select(Cust.Id, Set.Top.Box, Service.Cas.Code, Channel.Name) %>%
  unite(combined, c("Set.Top.Box", "Service.Cas.Code"))

ala_diff = merge(ls_al_com, ls_act_com, all.x = T, all.y = F)
ala_diff_flt = ala_diff %>%
  filter(is.na(ENTITY_CODE)) %>%
  separate(combined, c("STB", "cascode"))
...
customer_num = list_bouquet_dated %>%
  select(Customer.Number, Set.Top.Box) %>%
  unique() %>%
  group_by(Customer.Number) %>%
  summarise(STB.Count = n())
write.csv(customer_num, "Customer numbers with STB count.csv", row.names = F)

#####PLAN COUNT
plan_names = read.csv(sprintf(
  "https://drive.google.com/u/0/uc?id=17GoiwT4nWCn0J_7HJF0ZyL5Y0-JPNwOJ&export=download"
))
plan_names = plan_names %>%
  add_row(Plan.Name = c('DD Channels', 'Bronze basic', 'Odia FTA'))
ACTV_PLAN = list_active_flt %>%
  filter(PLAN_NAME %in% plan_names$Plan.Name) %>%
  select(PLAN_NAME, ENTITY_CODE, ENTITY_NAME, CUSTOMER_NBR, ) %>%
  unique()
ACTV_PLAN_CNT = ACTV_PLAN %>%
  group_by(PLAN_NAME) %>%
  summarise(Active.Count = n())
write.csv(ACTV_PLAN_CNT, "PLAN_COUNT_31082022.CSV")
##plan_count with LCO
lco_count = ACTV_PLAN %>%
  group_by(ENTITY_CODE, ENTITY_NAME, PLAN_NAME) %>%
  summarise(Active.Count = n())
...
ls_act_com = list_active_flt %>%
  select(CUSTOMER_NBR, VC, SERVICE_CODE, ENTITY_CODE) %>%
  unite(combined, c("VC", "SERVICE_CODE"))
cas_ent_com = cas_ent %>%
  select(Customer.Nbr, VC, Service.Code, Prov.System.Name) %>%
  unite(combined, c("VC", "Service.Code"))

cas_diff = merge(ls_act_com, cas_ent_com, all.x = T)

not_in_ent_rep = cas_diff %>%
  filter(is.na(Prov.System.Name)) %>%
  separate(combined, c('VC', 'SERVICE_CODE'))
...
list_active_old = list_active_flt %>%
  filter(!(PLAN_NAME %in% new_plan$PLAN_NAME))
...
lsactv_old_ala = lsactv_bc_fl[grepl("@", lsactv_bc_fl$SERVICE_NAME), ] %>%
  select(PLAN_NAME, SERVICE_NAME, PRI_STATE, CUSTOMER_NBR, Broadcaster)
lsactv_old_ala_chnname = merge(
  lsactv_old_ala,
  service_channel,
  by.x = 'SERVICE_NAME',
  by.y = 'Service.Name',
...
old_ala_final = lsactv_old_ala_chnname %>%
  select(PLAN_NAME, Channel, PRI_STATE, CUSTOMER_NBR, Broadcaster)
colnames(old_ala_final)[2] <- 'SERVICE_NAME'
lsactv_old_bq = lsactv_bc_fl[!grepl("@", lsactv_bc_fl$SERVICE_NAME), ] %>%
  select(PLAN_NAME, SERVICE_NAME, PRI_STATE, CUSTOMER_NBR, Broadcaster)
...
actv_new_serv_clean = actv_new_serv %>%
  select(PLAN_NAME, Bouquet, PRI_STATE, CUSTOMER_NBR, Broadcaster.Name) %>%
  unique()
...
LST_PCK_PIVOT = final_data %>%
  group_by(PRI_STATE, PLAN_NAME, SERVICE_NAME, Broadcaster) %>%
  summarise(Active.count = n())
write.csv(LST_PCK_PIVOT, "listactive_to_MSR_PLANWISE.CSV", row.names = F)
LST_PCK_PIVOT_SERVICE = final_data %>%
  group_by(PRI_STATE, SERVICE_NAME, Broadcaster) %>%
  summarise(Active.count = n())
write.csv(LST_PCK_PIVOT_SERVICE, "listactive_to_MSR.CSV", row.names = F)


######Package work
sports18_ala = list_active %>%
  filter(PLAN_NAME == 'Alacarte Plan') %>%
  filter(SERVICE_CODE == 'CH547')
plan_only = list_active %>%
  filter(PLAN_NAME %in% plan_names$Plan.Name) %>%
  select(
    ENTITY_CODE,
    ENTITY_NAME,
...
sports18_ala_wbaseplan = merge(
  sports18_ala,
  plan_only,
  all.x = T,
  all.y = F,
  by.x = 'CONTRACT_NUMBER',
  by.y = 'CONTRACT_NUMBER'
)
...
dueRenewal <- read.csv(file.path(dir_path, most_recent_file)) %>%
  select(Contract.Number, Contract.End.Date)
active_nt2 = merge(list_active, nt2Bouquets, all.x = F)
activeNt2EndDate = merge(
  active_nt2,
  dueRenewal,
  by.x = "CONTRACT_NUMBER",
  by.y = "Contract.Number"
)
activeNt2EndDate$Contract.End.Date = as.Date(
  activeNt2EndDate$Contract.End.Date,
  "%d/%m/%Y"
)
custListSlct1 = activeNt2EndDate %>%
  filter(Contract.End.Date == today()) %>%
  select(CONTRACT_NUMBER, SERVICE_CODE) %>%
  unique()
...
for (i in 1:nrow(custListSlct1)) {
  ...
  body1 <- paste0(
    "<REQUESTINFO>\r\n<CONTRACTINFO>\r\n <CONTRACTNO>",
    custListSlct1[i, "CONTRACT_NUMBER"],
    "</CONTRACTNO>\r\n <ORDERDATE>",
    mqdate,
    "</ORDERDATE>\r\n <EFFECTIVEDATE>",
    mqdate,
    "</EFFECTIVEDATE>\r\n <BILLFREQUENCY></BILLFREQUENCY>\r\n <SALESMANCODE></SALESMANCODE>\r\n <OUTLETS></OUTLETS>\r\n <NOTES></NOTES>\r\n <STATUS></STATUS>\r\n<DELETEINFO>\r\n<PLANCODE>",
    custListSlct1[i, "SERVICE_CODE"],
  ...
  ref_no1 <- paste0(
    format(runif(1, 510000, 100000000), scientific = FALSE),
    "a2sbhzd3"
  )
  url1 <- paste0(
    "https://meghbela-bcrm.magnaquest.com/RestService/RestService.svc/ModifyContract?referenceno=",
    ref_no1
  )
...
list_active_filtered = filter(
  list_active,
  PLAN_NAME %in% plan_names$Plan.Name
) %>%
  select(CUSTOMER_NBR, ENTITY_CODE, ENTITY_NAME, LCO_CITY, PLAN_NAME) %>%
  unique()
...
#########find DPO count####
list_active = read.csv(file.choose())
oplan_dpo = filter(list_active, PLAN_NAME %in% plan_names$Plan.Name) %>%
  select(CUSTOMER_NBR, PLAN_NAME) %>%
  unique()
colnames(oplan_dpo)[2] <- "Plan.Name"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Diamond_Dig_380 (Promotional)"
] <- "DIAMOND DIGITAL @ 380"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "DIAMOND DIGITAL @ 380 (Promotional)"
] <- "DIAMOND DIGITAL @ 380"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Gold_353 (Promotional)"
] <- "GOLD DIGITAL @ 353"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "GOLD DIGITAL @ 353 (Promotional)"
] <- "GOLD DIGITAL @ 353"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Hd_Dha_300(Promotional)"
] <- "HD DHAMAKA @ 300"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "HD DHAMAKA @ 300(Promotional)"
] <- "HD DHAMAKA @ 300"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Mb_Hd_Beng_455 (Promotional)"
] <- "MB BANGLA HD 1 @ 455"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Mb_Hd_Beng_550 (Promotional)"
] <- "MB BANGLA HD 2 @ 550"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "MB BANGLA HD 1 @ 455 (Promotional)"
] <- "MB BANGLA HD 1 @ 455"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "MB BANGLA HD 2 @ 550 (Promotional)"
] <- "MB BANGLA HD 2 @ 550"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Silver_Pow_276 (Promotional)"
] <- "Silver Digital Power @ 276"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Silver Digital Power @ 276 (Promotional)"
] <- "Silver Digital Power @ 276"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Silver Digital Special @ 276"
] <- "Silver Digital Power @ 276"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela_Bon_330 (Promotional)"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela Bonanza @ 330 (Promotional)"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela Bonanza Special @ 330"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela_Pac_185(Promotional)"
] <- "Meghbela Starter Pack @ 185"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela Starter Pack @ 185 (Promotional)"
] <- "Meghbela Starter Pack @ 185"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela Basic Pack @ 155 (Promo)"
] <- "Meghbela Basic Pack @ 155"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Platinum_Dig_450 (Promotional)"
] <- "PLATINUM DIGITAL @ 450"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "PLATINUM DIGITAL @ 450 (Promotional)"
] <- "PLATINUM DIGITAL @ 450"

oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Diamond_Dig_380"
] <- "DIAMOND DIGITAL @ 380"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Gold_Dig_353"
] <- "GOLD DIGITAL @ 353"
oplan_dpo$Plan.Name[oplan_dpo$Plan.Name == "Hd_Dha_300"] <- "HD DHAMAKA @ 300"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Mb_Hd_Beng_455"
] <- "MB BANGLA HD 1 @ 455"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Mb_Hd_Beng_550"
] <- "MB BANGLA HD 2 @ 550"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Silver_Pow_276"
] <- "Silver Digital Power @ 276"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela_Bon_330"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela_Pac_185"
] <- "Meghbela Starter Pack @ 185"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Platinum_Dig_450"
] <- "PLATINUM DIGITAL @ 450"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Meghbela_Sta_165"
] <- "Meghbela Bengali Starter @165"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Mb_Hd_Hin_455"
] <- "MB HINDI HD 1 @ 455"
oplan_dpo$Plan.Name[
  oplan_dpo$Plan.Name == "Mb_Hd_Hin_550"
] <- "MB HINDI HD 2 @ 550"
...
alacarte_names = c(
  ...
  "Star Gold 2 @ 5",
  "Star Sports 2 @ 19",
  "Star Sports Select 1 @ 19",
  "Star Sports 2 Hindi @ 19",
  "Star Sports Select 2 @ 10",
  "Star Gold Romance @ 3",
  "Star Gold Thrills @ 2",
  "Star Sports 1 Hindi @ 19",
  "Star Plus @ 19",
  "Star Jalsha HD @ 19",
  "Star Gold @ 19",
  "Star Movies @ 19",
  "Star Sports 3 @ 19",
  "Star Gold Select @ 7",
  "Star Sports 2 Hindi HD @ 19",
  "Star Sports Select HD1 @ 19",
  "Star Sports Select HD2 @ 15",
  "Star Bharat @ 15",
  "Star Sports HD2 @ 19",
  "Star Sports HD1 @ 19",
  "Star Sports 1 HD Hindi @ 19",
  "Star Movies Select HD @ 19",
  "Star Gold HD @ 19",
  "Star Sports Khel @ 1",
  "Star Gold 2 HD @ 8",
  "Star Plus HD @ 25",
  "Star Movies HD @ 19",
  "Star Sports 2 Tamil @ 19",
  "Star Gold Select HD @ 10",
  "Star Movies Select @ 10",
  "Star Bharat HD @ 19",
  "Star Sports 2 Telugu @ 19",
  "Star Sports 2 Tamil HD @ 19",
  "Star Sports 2 Telegu HD @ 19",
  "Star Kiran @ 15",
  "Star Pravah @ 6",
  "Jalsha Movies @ 15",
  "Jalsha Movies HD @ 19",
  "Colors Bangla Cinema @ 15",
  "Colors Bangla @ 7",
...
dpo_count = subset(
  list_active,
  grepl('^MBIL', list_active$Plan.Code, ignore.case = T)
)
dpo_count = dpo_count %>% group_by(Plan.Name) %>% summarise(Active = n())
dpo_count = dpo_count %>%
  mutate(
    Penetration = paste0(round(Active / act_cust_count$n * 100, 2), "%")
  ) %>%
  arrange(desc(Active))
...
ls_new_plan = subset(
  list_active,
  (grepl('^MBIL', list_active$PLAN_CODE, ignore.case = T))
)
plan_pivot = ls_new_plan %>%
  group_by(ENTITY_CODE, ENTITY_NAME, PLAN_NAME) %>%
  summarise(DPO_Count = n())
all_pivot = list_active %>%
  select(ENTITY_CODE, ENTITY_NAME, CUSTOMER_NBR) %>%
  unique() %>%
...
write.csv(all_lco, "LCO_DPO_count_June26.csv")


###old plans
plan = list_active %>%
  select(CUSTOMER_NBR, ENTITY_CODE, ENTITY_NAME, PLAN_NAME) %>%
  unique()
oplan_dpo = filter(plan, PLAN_NAME %in% plan_names$Plan.Name)

oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Diamond_Dig_380 (Promotional)"
] <- "DIAMOND DIGITAL @ 380"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "DIAMOND DIGITAL @ 380 (Promotional)"
] <- "DIAMOND DIGITAL @ 380"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Gold_353 (Promotional)"
] <- "GOLD DIGITAL @ 353"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "GOLD DIGITAL @ 353 (Promotional)"
] <- "GOLD DIGITAL @ 353"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Hd_Dha_300(Promotional)"
] <- "HD DHAMAKA @ 300"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "HD DHAMAKA @ 300(Promotional)"
] <- "HD DHAMAKA @ 300"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Mb_Hd_Beng_455 (Promotional)"
] <- "MB BANGLA HD 1 @ 455"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Mb_Hd_Beng_550 (Promotional)"
] <- "MB BANGLA HD 2 @ 550"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "MB BANGLA HD 1 @ 455 (Promotional)"
] <- "MB BANGLA HD 1 @ 455"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "MB BANGLA HD 2 @ 550 (Promotional)"
] <- "MB BANGLA HD 2 @ 550"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Silver_Pow_276 (Promotional)"
] <- "Silver Digital Power @ 276"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Silver Digital Power @ 276 (Promotional)"
] <- "Silver Digital Power @ 276"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Silver Digital Special @ 276"
] <- "Silver Digital Power @ 276"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela_Bon_330 (Promotional)"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela Bonanza @ 330 (Promotional)"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela Bonanza Special @ 330"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela_Pac_185(Promotional)"
] <- "Meghbela Starter Pack @ 185"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela Starter Pack @ 185 (Promotional)"
] <- "Meghbela Starter Pack @ 185"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela Basic Pack @ 155 (Promo)"
] <- "Meghbela Basic Pack @ 155"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Platinum_Dig_450 (Promotional)"
] <- "PLATINUM DIGITAL @ 450"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "PLATINUM DIGITAL @ 450 (Promotional)"
] <- "PLATINUM DIGITAL @ 450"

oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Diamond_Dig_380"
] <- "DIAMOND DIGITAL @ 380"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Gold_Dig_353"
] <- "GOLD DIGITAL @ 353"
oplan_dpo$PLAN_NAME[oplan_dpo$PLAN_NAME == "Hd_Dha_300"] <- "HD DHAMAKA @ 300"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Mb_Hd_Beng_455"
] <- "MB BANGLA HD 1 @ 455"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Mb_Hd_Beng_550"
] <- "MB BANGLA HD 2 @ 550"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Silver_Pow_276"
] <- "Silver Digital Power @ 276"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela_Bon_330"
] <- "Meghbela Bonanza @ 330"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela_Pac_185"
] <- "Meghbela Starter Pack @ 185"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Platinum_Dig_450"
] <- "PLATINUM DIGITAL @ 450"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Meghbela_Sta_165"
] <- "Meghbela Bengali Starter @165"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Mb_Hd_Hin_455"
] <- "MB HINDI HD 1 @ 455"
oplan_dpo$PLAN_NAME[
  oplan_dpo$PLAN_NAME == "Mb_Hd_Hin_550"
] <- "MB HINDI HD 2 @ 550"

plan_pivot = oplan_dpo %>%
  group_by(ENTITY_CODE, ENTITY_NAME, PLAN_NAME) %>%
  summarize(DPO_count = n())

####295 pack Compare count ####
ls_act_1st_month = read.csv(file.choose()) #OLD MONTH
ls_act_2nd_month = read.csv(file.choose()) #NEW MONTH
base_plan = read.csv(sprintf(
  "https://drive.google.com/u/0/uc?id=17GoiwT4nWCn0J_7HJF0ZyL5Y0-JPNwOJ&export=download"
))
base_plan = base_plan %>%
  add_row(Plan.Name = c('DD Channels', 'Bronze basic', 'Odia FTA'))

royal_act_2nd = ls_act_2nd_month %>%
  filter(PLAN_CODE == "MBILROYAL") %>%
  select(
    CUSTOMER_NBR,
    STB,
    SC,
...
base_plan_1st = ls_act_1st_month %>%
  filter(PLAN_NAME %in% base_plan$Plan.Name) %>%
  select(SC, PLAN_CODE, PLAN_NAME)
royal_merge = merge(
  royal_act_2nd,
  base_plan_1st,
  by.x = "SC",
...
royal_merge$PLAN_NAME.y[
  royal_merge$PLAN_NAME.y == "Bronze basic"
] <- "Bronze basic @ 155"
royal_merge$aggregator <- as.numeric(gsub(
  ".* @ (\\d+)$",
  "\\1",
  royal_merge$PLAN_NAME.y
))
royal_merge$aggregator[is.na(royal_merge$aggregator)] <- 0
royal_merge = royal_merge %>%
  mutate(
    Status = ifelse(aggregator > 325, 'Downgraded', 'Upgraded/Same'),
    .after = NULL
  )
royal_pivot = royal_merge %>%
  group_by(LCO_CITY, ENTITY_CODE, ENTITY_NAME, Status) %>%
  summarise(Count = n())
...
actvAla = lsactv %>%
  filter(PLAN_NAME == "Alacarte Plan") %>%
  filter(SERVICE_CODE %in% c("CH1")) %>%
  select(CUSTOMER_NBR, ENTITY_CODE, LCO_CITY, PLAN_NAME, SERVICE_NAME) %>%
  unique()
actvOther = lsactv %>%
  filter(!(PLAN_NAME == "Alacarte Plan")) %>%
  select(CUSTOMER_NBR, ENTITY_CODE, LCO_CITY, PLAN_NAME) %>%
  unique()
...
list_active <- mq_active_report()
abvData = list_active %>%
  filter(VC.length == "ABV") %>%
  select(STB, VC, CASCODE) %>%
  unique()
write.csv(x = abvData, file = file.choose())
